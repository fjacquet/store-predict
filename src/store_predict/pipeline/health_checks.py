...
def _check_missing_os(df: pd.DataFrame) -> list[HealthFinding]:
    ...
    return [
        HealthFinding(
            ...
            remediation="Export the RVTools file after ensuring VMware Tools is running on affected VMs — Tools populates the OS field.",
        )
    ]
...
def _check_zero_provisioned(df: pd.DataFrame) -> list[HealthFinding]:
    ...
    return [
        HealthFinding(
            ...
            affected_count=len(bad),
            affected_vms=tuple(bad["vm_name"].head(5).tolist()),
            remediation="Re-export the RVTools file; zero provisioned storage usually indicates a snapshot-only or powered-off VM missing disk data.",
        )
    ]
...
def _check_missing_cpu(df: pd.DataFrame) -> list[HealthFinding]:
    ...
    return [
        HealthFinding(
            ...
            affected_count=len(bad),
            affected_vms=tuple(bad["vm_name"].head(5).tolist()),
            remediation="Re-export after powering on these VMs — CPU data is absent for powered-off VMs in some RVTools versions.",
        )
    ]
...
def _check_missing_ram(df: pd.DataFrame) -> list[HealthFinding]:
    ...
    return [
        HealthFinding(
            ...
            affected_count=len(bad),
            affected_vms=tuple(bad["vm_name"].head(5).tolist()),
            remediation="Re-export after powering on these VMs — RAM data is absent for powered-off VMs in some RVTools versions.",
        )
    ]
...
def _check_high_powered_off_ratio(full_df: pd.DataFrame, _active: pd.DataFrame) -> list[HealthFinding]:
    ...
    return [
        HealthFinding(
            ...
            affected_vms=(),
            remediation="Confirm with the customer whether these are truly decommissioned VMs or if the export was taken during a maintenance window.",
        )
    ]
...
def _check_high_unknown_ratio(df: pd.DataFrame) -> list[HealthFinding]:
    ...
    return [
        HealthFinding(
            ...
            remediation="Review the VM Review page and manually classify these VMs, or add pattern rules to the classifier for your customer's naming convention.",
        )
    ]
...
def _check_large_unknown_vms(df: pd.DataFrame) -> list[HealthFinding]:
    ...
    return [
        HealthFinding(
            ...
            affected_count=len(bad),
            affected_vms=tuple(bad["vm_name"].head(5).tolist()),
            remediation="Classify these large VMs on the VM Review page before generating the final report — their DRR has a major impact on total required capacity.",
        )
    ]
...
def _check_iops_budget_exceeded(df: pd.DataFrame) -> list[HealthFinding]:
    ...
    return [
        HealthFinding(
            ...
            affected_count=len(bad),
            affected_vms=tuple(bad["vm_name"].head(5).tolist()),
            remediation="These VMs will require a dedicated datastore or a high-performance NVMe-tier datastore; discuss PowerStore T-series options with the customer.",
        )
    ]
...
def _check_no_cluster(df: pd.DataFrame) -> list[HealthFinding]:
    ...
    return [
        HealthFinding(
            ...
            affected_count=len(bad),
            affected_vms=tuple(bad["vm_name"].head(5).tolist()),
            remediation="These standalone hosts cannot use vMotion; confirm whether they are test/dev hosts or production hosts that should be clustered.",
        )
    ]
        ...
        if very_old_mask.any():
            findings.append(
                HealthFinding(
                    ...
                    remediation="Schedule an immediate ESXi upgrade to 7.0 U3 or later for this cluster — vHW below 14 is incompatible with modern vSphere features.",
                )
            )
        else:
            ...
            if old_mask.any():
                findings.append(
                    HealthFinding(
                        ...
                        cluster=str(cluster_name),
                        remediation="Plan an ESXi upgrade to 7.0 U3 or later for this cluster — vHW 14–16 cannot use the latest VM features and paravirtual drivers.",
                    )
                )
        ...
        if len(group) < 3:
            findings.append(
                HealthFinding(
                    ...
                    cluster=str(cluster_name),
                    remediation="Add at least one more ESXi host to this cluster to achieve N+1 HA; a 2-host cluster loses 50% capacity on a single host failure.",
                )
            )
    ...
    if not not_installed.empty:
        findings.append(
            HealthFinding(
                ...
                remediation="Install VMware Tools on these VMs before migration — without Tools, quiesced snapshots and guest OS commands are unavailable.",
            )
        )
    ...
    if not not_running.empty:
        findings.append(
            HealthFinding(
                ...
                remediation="Start the VMware Tools service on these VMs — Tools is installed but inactive, which disables quiesced snapshots and in-guest commands.",
            )
        )

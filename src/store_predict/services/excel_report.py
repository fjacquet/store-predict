...
import store_predict.i18n  # noqa: F401 — ensures i18n load_path and YAML config are initialised
...
def _t(key: str) -> str:
    """Translate key using the process-global locale set by generate_report_xlsx."""
    return str(_i18n.t(key))
...
def _write_summary_sheet(
    ...
    ws = wb.add_worksheet(_t("excel.sheet_summary"))

    # Header row
    ws.write_row(0, 0, [_t("excel.col_metric"), _t("excel.col_value")], header_fmt)
    ...
    write_metric(_t("pdf.total_vms"), summary.total_vms, int_fmt)
    write_metric(_t("pdf.total_cpus"), summary.total_cpus, int_fmt)
    write_metric(_t("pdf.total_memory"), summary.total_memory_mib / 1024.0, number_fmt)
    write_metric(_t("pdf.total_provisioned"), summary.total_provisioned_mib / 1024.0, number_fmt)
    write_metric(_t("pdf.total_in_use"), summary.total_in_use_mib / 1024.0, number_fmt)
    write_metric(_t("pdf.required_capacity"), summary.total_required_mib / 1024.0, number_fmt)
    write_metric(_t("pdf.avg_cpus"), summary.avg_vm_cpus, number_fmt)
    write_metric(_t("pdf.avg_memory"), summary.avg_vm_memory_mib / 1024.0, number_fmt)
    write_metric(_t("pdf.avg_storage"), summary.avg_vm_size_mib / 1024.0, number_fmt)
    write_metric(_t("pdf.weighted_drr"), summary.weighted_avg_drr, number_fmt)
    write_metric(_t("pdf.largest_vm"), summary.largest_vm_name)

    if summary.has_performance_data:
        write_metric(_t("pdf.total_avg_iops"), summary.total_avg_iops, number_fmt)
        write_metric(
            _t("pdf.hottest_vm"),
            f"{summary.max_vm_peak_iops_name} ({summary.max_vm_peak_iops:,.0f})",
        )
        write_metric(_t("pdf.peak_throughput"), summary.peak_throughput_mbs, number_fmt)
        write_metric(_t("pdf.iops_8k"), summary.total_iops_8k_equivalent, number_fmt)
...
def _write_breakdown_sheet(
    ...
    ws = wb.add_worksheet(_t("excel.sheet_breakdown"))

    headers = [
        _t("excel.col_category"),
        _t("excel.col_vms"),
        _t("excel.col_provisioned_gib"),
        _t("excel.col_avg_drr"),
        _t("excel.col_required_gib"),
    ]
...
def _write_vm_detail_sheet(
    ...
    ws = wb.add_worksheet(_t("excel.sheet_vm_detail"))

    headers = [
        _t("excel.col_vm_name"),
        _t("excel.col_workload"),
        _t("excel.col_drr"),
        _t("excel.col_provisioned_gib"),
        _t("excel.col_in_use_gib"),
        _t("excel.col_required_gib"),
    ]
    if summary.has_performance_data:
        headers.extend(
            [
                _t("excel.col_peak_iops"),
                _t("excel.col_avg_iops"),
                _t("excel.col_peak_mbs"),
                _t("excel.col_iops_8k"),
            ]
        )
    ws.write_row(0, 0, headers, header_fmt)
